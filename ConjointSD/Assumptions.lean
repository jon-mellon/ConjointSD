...
/-!
Minimal conjoint-design bundle used to feed both identification and SD-consistency
statements without duplicating assumptions.
-/
structure ConjointDesignAssumptions
    [MeasurableSpace Attr] (A : ℕ → Ω → Attr) (Y : Attr → Ω → ℝ) (Yobs : Ω → ℝ) : Prop where
  streamRand : ConjointRandomizationStream (μ := μ) (A := A) (Y := Y)
  ν_pos : ∀ x, (Measure.map (A 0) μ) {x} ≠ 0
  measYobs : Measurable Yobs
  measY : ∀ x, Measurable (Y x)
  consistency : ∀ ω, Yobs ω = Y (A 0 ω) ω
  bounded :
    ∀ x, ∃ C : ℝ, 0 ≤ C ∧ ∀ ω, |Y x ω| ≤ C
