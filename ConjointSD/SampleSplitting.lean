...
theorem sdHat_fixed_m_tendsto_ae_attrSD
  ...
  have hMean :
      ...
      attrMean ν_pop (gHat g θhat m) := by
    simpa [kappaDesign, hLaw.identA 0] using
      (designMeanZ_Zcomp_eq_attrMean (κ := ρ) (A := A) (g := gHat g θhat m)
        (hA0 := hLaw.measA 0) (hg := h.hMeasG))
  have hM2 :
      designM2Z (κ := ρ)
        (Z := Zcomp (A := A) (g := gHat g θhat m)) =
      attrM2 ν_pop (gHat g θhat m) := by
    simpa [kappaDesign, hLaw.identA 0] using
