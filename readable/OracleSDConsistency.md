...
This file restates the [standard deviation](jargon_standard_deviation.md) [convergence](jargon_convergence.md) result so the limit is written in terms of the attribute distribution `nu`.
...
- Uses the PopulationBridge result to rewrite `popSDZ` as `popSDAttr nu g` when `A 0` has distribution `nu`.
