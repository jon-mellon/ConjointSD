...
- Adds end-to-end OLS wrappers that derive well-specification from `NoInteractions` + `FullMainEffectsTerms`, use randomized assignment for identification, and then apply the respondent-sampling LLN bridge (`RespondentSamplingLLN`) to target the population SD for the total score (`paper_sd_total_sequential_consistency_to_true_target_ae_of_paper_ols_design_ae_of_NoInteractions_of_randomization`) alongside the block component wrapper (`paper_sd_blocks_sequential_consistency_to_true_target_ae_of_paper_ols_design_ae_of_NoInteractions_of_randomization`).
