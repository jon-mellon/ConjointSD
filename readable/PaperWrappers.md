...
- Wraps the identification results so they read as "[conditional mean](jargon_conditional_mean.md) identifies the potential [mean](jargon_mean.md)" and "difference in conditional means identifies the treatment contrast."
...
- If the total model score at `theta0` matches a linear term model and that model is well-specified for `gStar`, then the [standard deviation](jargon_standard_deviation.md) [consistency](jargon_consistency.md) targets `gStar`.

Section 4d: no-interactions corollary
- Uses the "no interactions" assumption to produce well-specification automatically and then applies the same [standard deviation](jargon_standard_deviation.md) [consistency](jargon_consistency.md) bridge.
