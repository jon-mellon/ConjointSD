...
- `ScoreAssumptions` adds [measurability](jargon_measurable.md) and [integrable](jargon_integrable.md) conditions for `g`.

Main results:
- If `g` is bounded, then the [integrable](jargon_integrable.md) assumptions hold automatically.
- `iidAssumptions_Zcomp` shows that the score process `Zcomp` inherits [IID](jargon_iid.md) properties from `A`.
- `sd_component_consistent` proves the empirical [standard deviation](jargon_standard_deviation.md) of `g(A i)` [converges](jargon_convergence.md) to the [population](jargon_population.md) [SD](jargon_standard_deviation.md).
