...
This file handles the evaluation stage in a sample-splitting setup. It now uses `ProbMeasureAssumptions` and the bundled `MapLawAssumptions` when translating population-space SDs to attribute-space SDs.

Key definitions:
- `SplitEvalAssumptions` bundles the assumptions needed to treat the [plug-in](jargon_plug_in.md) score at training index `m` as fixed and apply the [standard deviation](jargon_standard_deviation.md) [convergence](jargon_convergence.md) result; measurability of `A 0` is inherited from the bundled `PopIID`.
