...
This file handles the evaluation stage in a sample-splitting setup. It uses `IsProbabilityMeasure` for the target attribute distribution `ν_pop` and assumes `EvalAttrLawEqPop` so the evaluation draws are IID with law `ν_pop` under the evaluation law `ρ`. The training data that produced `θhat` is not modeled here.
