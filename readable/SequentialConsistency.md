...
This file proves a two-stage [convergence](jargon_convergence.md) statement for [standard deviation](jargon_standard_deviation.md) estimation with sample splitting. It uses bundled assumption records for probability and convergence (`ProbMeasureAssumptions`, `ThetaTendstoAssumptions`, `EpsilonAssumptions`) and targets the attribute-distribution SD under `ν`, with `EvalWeightMatchesAttrMoments` matching weighted evaluation moments under `μ` to the target moments. The training distribution that produces `θhat` is left abstract.
