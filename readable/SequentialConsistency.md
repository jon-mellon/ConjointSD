...
This file proves a two-stage [convergence](jargon_convergence.md) statement for [standard deviation](jargon_standard_deviation.md) estimation with sample splitting. It uses `ProbMeasureAssumptions` and `EpsilonAssumptions`, and takes direct plug‑in moment convergence (`PlugInMomentAssumptions`) as input. The target is the attribute-distribution SD under `ν`, with `EvalAttrLawEqPop` asserting that the evaluation attribute law equals `ν` (uniform weights). The evaluation assumptions are now boundedness-based (`SplitEvalWeightAssumptionsBounded`), which then derive the score/integrability conditions internally. The training distribution that produces `θhat` is left abstract.
...
- `totalErr`: the gap between the evaluation estimator (uniform weights) and the oracle [standard deviation](jargon_standard_deviation.md).
...
2) As `m` grows, `trainErr` goes to 0, using the assumed plug‑in mean/second‑moment [convergence](jargon_convergence.md).
