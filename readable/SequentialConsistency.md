...
It uses the notion of [sequential consistency](jargon_sequential_consistency.md).

Definitions:
- `sdEst`: the evaluation-stage [standard deviation](jargon_standard_deviation.md) estimator using a fixed training index `m` and evaluation size `n`.
...
- `trainErr`: the gap between the plug-in [standard deviation](jargon_standard_deviation.md) at training index `m` and the oracle SD.
