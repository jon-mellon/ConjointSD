...
score regularity, and then packages the full LLN structure used in the main transport step.

Key pieces:
- `subject_lln_gPop_of_iid`: uses the strong law of large numbers to show that the subject-average
  score converges a.e. to `gPop` under `SubjectSamplingIID` and `SubjectScoreAssumptions`.
