...
This file defines the "true" [block](jargon_block.md)-level score coming from a [linear-in-terms](jargon_linear_in_terms.md) model, then shows how the block [standard deviation](jargon_standard_deviation.md) [convergence](jargon_convergence.md) results can be targeted at those true block scores. The theorem statements use the usual probability and convergence bundles and target the attribute distribution `ν`, with `EvalAttrLaw` connecting the evaluation draw under `μ` to that target law.
