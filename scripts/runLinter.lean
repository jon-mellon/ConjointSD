...
/--
Parse args list for `runLinter` and return a pair of the update and specified module arguments.
-/
...
/--
Usage: `runLinter [--update] [ConjointSD]`

Runs the linters on all declarations in the given module (or all root modules of Lake
default targets if no module is specified). If `--update` is set, the `nolints` file is
updated to remove any declarations that no longer need to be nolinted.
-/
unsafe def main (args : List String) : IO Unit := do
  ...
  let (update, specifiedModule) ← match linterArgs with
    ...
    | Except.error msg => do
      ...
      IO.eprintln "Usage: runLinter [--update] [ConjointSD]"
      IO.Process.exit 1
