...
# Theorems/lemmas used only for derived-assumption detection.
derivation_lines <- lines[grepl("^(theorem|lemma) ConjointSD\\.", lines)]
extract_name <- function(line) {
  name <- sub("^(theorem|lemma) ConjointSD\\.([^: ]+).*$", "\\2", line)
  sub("\\.\\{.*$", "", name)
}
derivations <- unique(vapply(derivation_lines, extract_name, character(1)))
...
block_for <- function(name, lines) {
  start_pat <- paste0("^theorem ConjointSD\\.", name, "(\\.|\\s)")
...
# Extract #print blocks for theorems/lemmas (for derived assumptions only).
block_for_any <- function(name, lines) {
  start_pat <- paste0("^(theorem|lemma) ConjointSD\\.", name, "(\\.|\\s)")
  idx <- grep(start_pat, lines)
  if (length(idx) == 0) return(character(0))
  start <- idx[1]
  next_idx <- grep("^(theorem|lemma|structure) ConjointSD\\.", lines[(start + 1):length(lines)])
  end <- if (length(next_idx) == 0) length(lines) else start + next_idx[1] - 1
  lines[start:end]
}
...
derivation_blocks <- setNames(lapply(derivations, block_for_any, lines = lines), derivations)
...
find_deps <- function(name, block, all_names) {
  ...
  hits <- vapply(
    all_names,
    function(nm) grepl(paste0("ConjointSD\\.", nm, "\\b"), text, perl = TRUE),
    logical(1)
  )
...
assumption_defs_in_block <- function(text) {
  ...
  for (nm in assumption_defs) {
    if (grepl(paste0("ConjointSD\\.", nm, "\\b"), text, perl = TRUE)) hits <- c(hits, nm)
  }
  hits
}
...
assumptions_in_block <- function(block) {
  ...
  for (s in structures) {
    if (grepl(paste0("ConjointSD\\.", s, "\\b"), text, perl = TRUE)) {
...
signature_text_from <- function(name, block_map) {
  block <- block_map[[name]]
  if (length(block) == 0) return(character(0))
  sig_end <- which(grepl(":=", block, fixed = TRUE))
  sig_end <- if (length(sig_end) == 0) length(block) else sig_end[1]
  block[seq_len(sig_end)]
}

signature_type <- function(sig_lines) {
  if (length(sig_lines) == 0) return("")
  sig <- paste(sig_lines, collapse = "\n")
  sub("^(theorem|lemma)\\s+ConjointSD\\.[^:]+:\\s*", "", sig)
}

signature_conclusion <- function(type_text) {
  parts <- strsplit(type_text, "→", fixed = TRUE)[[1]]
  tail(trimws(parts), 1)
}

assumptions_in_sig <- function(sig_lines) {
  if (length(sig_lines) == 0) return(character(0))
  type_text <- signature_type(sig_lines)
  if (length(type_text) == 0) return(character(0))
  parts <- strsplit(type_text, "→", fixed = TRUE)[[1]]
...
theorem_sigs <- setNames(lapply(theorems, signature_text_from, block_map = blocks), theorems)
theorem_types <- setNames(lapply(theorem_sigs, signature_type), theorems)
theorem_conclusions <- setNames(lapply(theorem_types, signature_conclusion), theorems)
theorem_assumptions <- setNames(lapply(theorem_sigs, assumptions_in_sig), theorems)

derivation_sigs <- setNames(lapply(derivations, signature_text_from, block_map = derivation_blocks), derivations)
derivation_types <- setNames(lapply(derivation_sigs, signature_type), derivations)
derivation_conclusions <- setNames(lapply(derivation_types, signature_conclusion), derivations)
derivation_assumptions <- setNames(lapply(derivation_sigs, assumptions_in_sig), derivations)

filter_derived_assumptions <- function(assumps) {
  if (length(assumps) == 0) return(assumps)
  struct_names <- unique(vapply(strsplit(assumps, " "), function(parts) parts[1], character(1)))
  for (s in struct_names) {
    if (!nzchar(s)) next
    if (s %in% structures) {
      derivable <- FALSE
      for (thm in names(derivation_conclusions)) {
        concl <- derivation_conclusions[[thm]]
        if (length(concl) == 0 || is.na(concl)) next
        if (!grepl(paste0("\\b", s, "\\b"), concl)) next
        thm_assumps <- derivation_assumptions[[thm]]
        if (any(grepl(paste0("^", s, "\\b"), thm_assumps))) next
        if (all(thm_assumps %in% assumps)) {
          derivable <- TRUE
          break
        }
      }
      if (derivable) {
        assumps <- assumps[!grepl(paste0("^", s, "\\b"), assumps)]
      }
    } else {
      derivable <- FALSE
      for (thm in names(derivation_conclusions)) {
...
if (!(target_thm %in% all_names)) target_thm <- theorems[[1]]
chain_nodes <- unique(c(target_thm, all_deps[[target_thm]]))
chain_nodes <- chain_nodes[chain_nodes %in% theorems]
global_assumps <- names(which(assumption_all[[target_thm]]))
global_assumps <- filter_derived_assumptions(global_assumps)
...
assump_edges <- lapply(chain_nodes, function(thm) {
  assumps <- assumptions_in_sig(theorem_sigs[[thm]])
  assumps <- filter_derived_assumptions(assumps)
  assumps <- intersect(assumps, global_assumps)
...
target_block <- "paper_sd_blocks_sequential_consistency_to_true_target_ae_of_paper_ols_design_ae_of_NoInteractions_of_randomization"
if (!(target_block %in% all_names)) target_block <- theorems[[1]]
block_nodes <- unique(c(target_block, all_deps[[target_block]]))
block_nodes <- block_nodes[block_nodes %in% theorems]
global_assumps_block <- names(which(assumption_all[[target_block]]))
global_assumps_block <- filter_derived_assumptions(global_assumps_block)

edge_rows <- lapply(block_nodes, function(thm) {
  deps <- intersect(raw_deps[[thm]], block_nodes)
  if (length(deps) == 0) return(NULL)
  data.frame(from = deps, to = thm, stringsAsFactors = FALSE)
})
edges <- do.call(rbind, edge_rows)

assump_edges <- lapply(block_nodes, function(thm) {
  assumps <- assumptions_in_sig(theorem_sigs[[thm]])
  assumps <- filter_derived_assumptions(assumps)
  assumps <- intersect(assumps, global_assumps_block)
  if (length(assumps) == 0) return(NULL)
  data.frame(from = assumps, to = thm, stringsAsFactors = FALSE)
})
assump_edges <- do.call(rbind, assump_edges)
...
if ((is.null(edges) || nrow(edges) == 0) && length(assump_nodes) == 0) {
  plot.new()
  text(0.5, 0.5, "No OLS SD block dependencies found.")
} else {
  pretty_core <- setNames(vapply(block_nodes, pretty_name, character(1)), block_nodes)
  core_labels <- vapply(
    block_nodes,
    function(nm) pretty_core[[nm]],
    character(1)
  ...
  out_dir <- "dependency_tables_files"
  if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
  out_png <- file.path(out_dir, "ols_sd_block_consistency_dag.png")
  rsvg::rsvg_png(charToRaw(svg), file = out_png)
  knitr::include_graphics(out_png)
}
